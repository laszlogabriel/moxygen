    ...
    case 'object':
      if (Array.isArray(element)) {
        ...
        switch (element['#name']) {
          case 'ref':
            return (
              s + markdown.refLink(toMarkdown(element.$$), element.$.refid)
            );
          case '__text__':
            s = element._;
            break;
          ...
          case 'title':
            var level = '#'.repeat(
              context[context.length - 1]['#name'].slice(-1)
            );
            s = '\n#' + level + ' ' + element._ + '\n';
            break;
...
module.exports = {
    ...
    switch (member.kind) {
      ...
      case 'function':
        ...
        if (memberdef.templateparamlist) {
          ...
          if (
            ...
            memberdef.templateparamlist.param
          ) {
            memberdef.templateparamlist[0].param.forEach(function (
              param,
              argn
            ) {
              m = m.concat(argn == 0 ? [] : ',');
        ...
        m = m.concat(
          memberdef.argsstring[0]._.match(/noexcept$/) ? ' noexcept' : ''
        );
        m = m.concat(
          memberdef.argsstring[0]._.match(/=\s*delete$/) ? ' = delete' : ''
        );
        m = m.concat(
          memberdef.argsstring[0]._.match(/=\s*default/) ? ' = default' : ''
        );
        break;
      ...
      case 'enum':
        ...
        m = m.concat([
          member.kind,
          ' ',
          markdown.refLink(member.name, member.refid),
        ]);
        break;
  ...
  parseCompound: function (compound, compounddef) {
    ...
    if (compounddef.sectiondef) {
      compounddef.sectiondef.forEach(
        function (section) {
          // switch (section.$['kind']) {
          //   case 'define':
          //   case 'enum':
    ...
    switch (compound.kind) {
      ...
      case 'group':
        ...
        if (compounddef.innerclass) {
          compounddef.innerclass.forEach(
            function (innerclassdef) {
              switch (compound.kind) {
                case 'namespace':
                  if (this.references[innerclassdef.$.refid])
                    this.assignToNamespace(
                      compound,
        ...
        if (compounddef.innernamespace) {
          ...
          compounddef.innernamespace.forEach(
            function (namespacedef) {
              if (compound.kind == 'group') {
                // log.verbose('Assign namespace ' + namespacedef.$.refid + ' to group ' + compound.name);
                if (this.references[innerclassdef.$.refid])
                  this.assignNamespaceToGroup(
                    compound,
  ...
  parseIndex: function (root, index, options) {
    index.forEach(
      function (element) {
        var doxygen,
          compound = root.find(element.$.refid, element.name[0], true);
        ...
        if (compound.kind !== 'file') {
          logger.verbose(
            'Parsing ' + path.join(options.directory, compound.refid + '.xml')
          );
  ...
  loadIndex: function (options, callback) {
    ...
    fs.readFile(
      path.join(options.directory, 'index.xml'),
      'utf8',
      function (err, data) {
        if (err) {
        ...
        xmlParser.parseString(
          ...
          function (err, result) {
            ...
            this.parseIndex(
              this.root,
              result.doxygenindex.compound.sort(function (a, b) {
